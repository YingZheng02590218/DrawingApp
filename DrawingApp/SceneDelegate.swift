...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        // MARK: 他アプリからPDFファイルを受け取る　アプリ外部から共有された場合　図面PDFファイルを取り込む　iCloud Container にプロジェクトフォルダを作成　iOS13から アプリ未起動
        ...
        if connectionOptions.urlContexts.count == 1 {
            ...
            let openInPlace = connectionOptions.urlContexts.first?.options.openInPlace
            let eventAttribution = connectionOptions.urlContexts.first?.options.eventAttribution
            if let _ = connectionOptions.urlContexts.first?.options.openInPlace {
                // ファイルが共有されたときの処理
    ...
    // MARK: 他アプリからPDFファイルを受け取る　アプリ外部から共有された場合　図面PDFファイルを取り込む　iCloud Container にプロジェクトフォルダを作成　iOS13から　バックグラウンド
